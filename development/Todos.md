...
- Python バージョン: 3.14.2（2025-12-25にアップグレード）
  - Python 3.13.7 → 3.14.2
  - テスト実行時間: 195.68秒 → 166.01秒（**15%高速化**）
  - 完全な互換性: 479/530テスト成功（リグレッションなし）
- 主な技術スタック: Pydantic v2, pytest-xdist, structlog, cryptography, aiofiles
- アーキテクチャ: Polylith monorepo

### Python 3.14の主な改善点
- **パフォーマンス向上**: 3.13比で5-15%高速化（実測15%）
- **JITコンパイラ**: 実験的機能の安定性向上
- **型システム**: PEP 742（TypeIs）、PEP 747（TypeForm）サポート
- **エラーメッセージ**: より詳細なトレースバック情報
- **参考資料**: https://docs.python.org/3.14/whatsnew/3.14.html
