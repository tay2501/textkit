...
class TestWidthConverter:
    ...
    def test_space_conversion(self):
        ...
        full_space = "　"  # Full-width space (U+3000)
    ...
    def test_special_katakana_combinations(self):
        ...
        full_width_complex = "ガギグゲゴザジズゼゾダヂヅデドバビブベボパピプペポ"
    ...
    @pytest.mark.parametrize(
        "direction,input_text,expected_contains",
        [
            ("fh", "ＡＢＣ", "ABC"),
            ("hf", "ABC", "Ａ"),
            ("fh", "アイウ", "ｱｲｳ"),
    ...
    def test_parametrized_conversion(
        self, direction: str, input_text: str, expected_contains: str
    ):
        """Test conversion with various parameter combinations."""
        result = self.converter.convert_width(input_text, direction)
        assert expected_contains in result or any(
            char in result for char in expected_contains
        )
