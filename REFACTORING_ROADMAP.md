...
## 🎯 Phase 1: Critical Security Fixes (P0 - 即座に実施)
...
# =============================================================================
# Cryptography - Security Configuration
# =============================================================================
# REQUIRED: Private key encryption passphrase (minimum 32 characters)
# Generate with: python -c "import secrets; print(secrets.token_urlsafe(48))"
TEXTKIT_KEY_PASSPHRASE=<your-secure-passphrase-here>

# Optional: Custom key directory path
# TEXTKIT_KEY_DIRECTORY=rsa
EOF
```

**Step 1.1.2: コード修正 (1.5時間)**

**ファイル:** `components/crypto_engine/core.py`

```python
# ADD: Import for environment variable handling
import os
...
class CryptographyManager:
    ...
    # Module-level constants (following N806 naming convention)
    ...
    def __init__(self, config_manager: ConfigManagerProtocol | None = None) -> None:
        """Initialize the cryptography manager with secure defaults.
        ...
        # Load RSA configuration with secure defaults
        self.rsa_config = {
            "key_size": self.DEFAULT_KEY_SIZE,
            "public_exponent": 65537,
            "aes_key_size": self.DEFAULT_AES_KEY_SIZE,
            "nonce_size": self.DEFAULT_GCM_NONCE_SIZE,
            "key_directory": "rsa",
        ...
        if config_manager:
            try:
                ...
                # Use defaults if config loading fails (EAFP principle)
                pass

        # Set up key paths with proper directory creation
    ...
    def _get_key_passphrase(self) -> bytes:
        ...
        Raises:
            CryptographyError: If passphrase environment variable is not set
        ...
        """
        passphrase_var = self.rsa_config["passphrase_env_var"]
        ...
        if len(passphrase) < 32:
            raise CryptographyError(
                f"Passphrase too short (minimum 32 characters, got {len(passphrase)})",
                {"required_length": 32, "actual_length": len(passphrase)}
            )
    ...
    def _save_key_pair(
        ...
        """Save RSA key pair to PEM files with secure encryption.
    ...
    def _load_key_pair(self) -> tuple[RSAPrivateKey, RSAPublicKey]:
        ...
        try:
            ...
            private_key = serialization.load_pem_private_key(
                private_pem,
                password=passphrase,  # CHANGED: was None (SECURITY FIX)
            ...
            raise CryptographyError(
                "Failed to load private key. Check passphrase.",
                {
                    ...
                    "hint": "Verify TEXTKIT_KEY_PASSPHRASE environment variable",
                },
            ) from e
        except Exception as e:
            raise CryptographyError(
...
**Step 1.1.3: テストの更新 (1時間)**

**ファイル:** `test/components/crypto_engine/test_core.py`
...
import secrets
from pathlib import Path
from components.crypto_engine.core import CryptographyManager
from textkit.exceptions import CryptographyError


@pytest.fixture
def test_passphrase():
    """Generate secure test passphrase."""
    return secrets.token_urlsafe(48)


@pytest.fixture
def crypto_manager_with_passphrase(tmp_path, test_passphrase, monkeypatch):
    """Create CryptographyManager with temporary directory and passphrase."""
    # Set environment variable for test
    monkeypatch.setenv("TEXTKIT_KEY_PASSPHRASE", test_passphrase)

    # Create manager with temporary key directory
    manager = CryptographyManager()
    manager.rsa_config["key_directory"] = str(tmp_path / "rsa")
    manager.key_directory = Path(manager.rsa_config["key_directory"])
    manager.key_directory.mkdir(parents=True, exist_ok=True)
...
class TestSecureKeyManagement:
    """Test secure key management with passphrase protection."""

    def test_passphrase_required(self, tmp_path, monkeypatch):
        """Test that passphrase environment variable is required."""
        # Remove passphrase env var
        monkeypatch.delenv("TEXTKIT_KEY_PASSPHRASE", raising=False)

        manager = CryptographyManager()
        manager.key_directory = tmp_path / "rsa"
        manager.private_key_path = manager.key_directory / "private_key.pem"
        manager.public_key_path = manager.key_directory / "public_key.pem"

        with pytest.raises(CryptographyError, match="passphrase not set"):
            manager.ensure_key_pair()

    def test_passphrase_minimum_length(self, tmp_path, monkeypatch):
        """Test that passphrase must be at least 32 characters."""
        # Set too-short passphrase
        monkeypatch.setenv("TEXTKIT_KEY_PASSPHRASE", "tooshort")
    ...
    def test_private_key_encryption(self, crypto_manager_with_passphrase):
        """Test that private key is encrypted on disk."""
        manager = crypto_manager_with_passphrase

        # Generate key pair
        private_key, public_key = manager.ensure_key_pair()

        # Verify private key file exists
        assert manager.private_key_path.exists()

        # Read raw PEM content
        private_pem = manager.private_key_path.read_text()

        # Encrypted private keys should have "ENCRYPTED" in header
        assert "ENCRYPTED" in private_pem, "Private key should be encrypted"

        # Verify file permissions (owner read/write only)
        assert oct(manager.private_key_path.stat().st_mode)[-3:] == "600"

    def test_key_loading_with_correct_passphrase(
        self, crypto_manager_with_passphrase
    ):
        """Test that keys can be loaded with correct passphrase."""
        manager = crypto_manager_with_passphrase
        ...
        manager2 = CryptographyManager()
        manager2.key_directory = manager.key_directory
        manager2.private_key_path = manager.private_key_path
        manager2.public_key_path = manager.public_key_path

        # Should load successfully
        private_key, public_key = manager2._load_key_pair()
        assert private_key is not None
        assert public_key is not None

    def test_key_loading_with_incorrect_passphrase(
        self, crypto_manager_with_passphrase, monkeypatch
    ):
        """Test that loading fails with incorrect passphrase."""
        manager = crypto_manager_with_passphrase

        # Generate and save keys with correct passphrase
        manager.ensure_key_pair()
        ...
        # Should fail to load
        with pytest.raises(CryptographyError, match="incorrect_passphrase"):
            manager2._load_key_pair()

    def test_encryption_decryption_with_secure_keys(
        self, crypto_manager_with_passphrase
    ):
        """Test end-to-end encryption/decryption with secure key storage."""
        manager = crypto_manager_with_passphrase

        # Test data
        original_text = "Sensitive data requiring encryption"
...
**Step 1.1.4: ドキュメント更新 (30分)**

**ファイル:** `README.md` (セキュリティセクション追加)
...
The toolkit uses RSA-4096 for key exchange and AES-256-GCM for data encryption. Private keys are protected with passphrase-based encryption.
...
3. Verify setup:
```bash
uv run python -c "from components.crypto_engine.core import CryptographyManager; m = CryptographyManager(); print('✓ Crypto configured')"
```
...
- Store production passphrases in secret management systems (HashiCorp Vault, AWS Secrets Manager)
```

**Step 1.1.5: 検証 (30分)**

```bash
# 1. Generate test passphrase
export TEXTKIT_KEY_PASSPHRASE=$(python -c "import secrets; print(secrets.token_urlsafe(48))")

# 2. Run security-specific tests
PYTHONPATH=. uv run pytest test/components/crypto_engine/test_core.py::TestSecureKeyManagement -v

# 3. Verify encrypted key format
PYTHONPATH=. uv run python -c "
from components.crypto_engine.core import CryptographyManager
m = CryptographyManager()
m.ensure_key_pair()
with open('rsa/private_key.pem', 'r') as f:
    content = f.read()
    assert 'ENCRYPTED' in content, 'Private key must be encrypted'
    print('✓ Private key is encrypted')
"

# 4. Test incorrect passphrase handling
TEXTKIT_KEY_PASSPHRASE="wrong_passphrase" PYTHONPATH=. uv run python -c "
from components.crypto_engine.core import CryptographyManager
from textkit.exceptions import CryptographyError
try:
    m = CryptographyManager()
    m._load_key_pair()
    print('✗ Should have failed with incorrect passphrase')
except CryptographyError as e:
    print('✓ Correctly rejected incorrect passphrase')
"

# 5. Run full crypto test suite
PYTHONPATH=. uv run pytest test/components/crypto_engine/ -v --tb=short
```
...
def encrypt_text(self, text: str) -> str:
    ...
    6. Combine: encrypted_aes_key + nonce + tag + encrypted_data
    ...
    try:
        ...
        rsa_key_size_bytes = self.rsa_config["key_size"] // 8  # 4096 / 8 = 512
...
def decrypt_text(self, encrypted_text: str) -> str:
    ...
    Example:
        ...
        >>> plaintext = manager.decrypt_text(encrypted)
    ...
    try:
        ...
        cipher = Cipher(
            algorithms.AES(aes_key),
            modes.GCM(nonce, tag),  # Tag verification is automatic
            backend=default_backend()
        )
        ...
        raise CryptographyError(
            f"Decryption failed: {e}",
            {
                ...
                "error_type": type(e).__name__,
                "hint": "Data may be corrupted or tampered with"
...
**Step 1.2.2: crypto.pyの削除と統合 (30分)**

```bash
# 1. Verify no external dependencies on crypto.py
rg -g "*.py" "from.*crypto_engine.crypto import" .
rg -g "*.py" "import.*crypto_engine.crypto" .

# 2. If no dependencies found, remove crypto.py
# (Manual review required to confirm)

# 3. Update component __init__.py
cat > components/crypto_engine/__init__.py << 'EOF'
...
from .core import CryptographyManager

__all__ = ["CryptographyManager"]
EOF
```

**Step 1.2.3: 既存データの移行スクリプト作成 (1時間)**

**ファイル:** `bin/migrate_crypto_cbc_to_gcm.py` (新規作成)

```python
#!/usr/bin/env python
"""Migrate encrypted data from CBC mode to GCM mode.

Usage:
    python bin/migrate_crypto_cbc_to_gcm.py --input encrypted.txt --output migrated.txt

WARNING: This script requires both old (CBC) and new (GCM) crypto implementations.
         Only run during migration period.
"""
...
def migrate_encrypted_file(input_path: Path, output_path: Path) -> None:
    """Migrate encrypted file from CBC to GCM mode.

    Args:
        input_path: Path to CBC-encrypted file
        output_path: Path to save GCM-encrypted file
    """
    # TODO: Implement migration logic
    # 1. Load old CryptographyManager (CBC mode)
    # 2. Decrypt with old manager
    # 3. Encrypt with new manager (GCM mode)
    # 4. Save to output file

    raise NotImplementedError(
        "Migration script requires preservation of old CBC implementation. "
        "Implement if you have existing CBC-encrypted data."
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate encrypted data from CBC to GCM mode"
    )
    parser.add_argument("--input", required=True, help="Input file (CBC encrypted)")
    parser.add_argument("--output", required=True, help="Output file (GCM encrypted)")
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.dry_run:
        print(f"[DRY RUN] Would migrate: {input_path} -> {output_path}")
        sys.exit(0)

    try:
        migrate_encrypted_file(input_path, output_path)
...
**Step 1.2.4: テストの更新 (30分)**

```bash
# 1. Remove CBC-specific tests
# (Review test/components/crypto_engine/ for CBC references)

# 2. Add GCM-specific tests
PYTHONPATH=. uv run pytest test/components/crypto_engine/ -v -k "gcm or encrypt"

# 3. Verify no CBC references remain
rg -g "*.py" "modes\.CBC" components/crypto_engine/
# Expected: No results
```
...
pem = Path('rsa/private_key.pem').read_text()
assert 'ENCRYPTED' in pem, 'Private key must be encrypted'
print('✓ Private key encryption verified')
"

# 2. Test GCM encryption/decryption
PYTHONPATH=. uv run python -c "
from components.crypto_engine.core import CryptographyManager

m = CryptographyManager()
original = 'Test sensitive data 日本語'
encrypted = m.encrypt_text(original)
decrypted = m.decrypt_text(encrypted)
assert decrypted == original, 'Encryption roundtrip failed'
print('✓ GCM encryption/decryption verified')
"
...
PYTHONPATH=. uv run pytest test/components/crypto_engine/ -v --cov --tb=short

# 5. Security audit with ruff
uv run ruff check components/crypto_engine/ --select S  # Security rules
```

**ロールバック手順:**

```bash
# If Phase 1 fails, rollback with git:
git checkout HEAD -- components/crypto_engine/core.py
git checkout HEAD -- test/components/crypto_engine/
