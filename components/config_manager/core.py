...
class ConfigurationManager:
    ...
    def load_hotkey_config(self) -> ConfigDict:
        ...
        if self._hotkey_config is None:
            self._hotkey_config = self._load_json_file(
                "hotkey_config.json", default_content=self._get_default_hotkey_config()
            )
        return self._hotkey_config.copy()

    def _load_json_file(
        self, filename: str, default_content: ConfigDict | None = None
    ) -> ConfigDict:
    ...
    def validate_config(self, config: ConfigDict, filename: str) -> bool:
        ...
        if not config:
            raise ConfigurationError(
                f"Empty configuration in {filename}", {"filename": filename}
            )
    ...
    def get_config_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "files": {
                "transformation_rules.json": (
                    self.config_dir / "transformation_rules.json"
                ).exists(),
                "security_config.json": (
                    self.config_dir / "security_config.json"
                ).exists(),
