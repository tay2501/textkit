...
def get_cpu_executor() -> ThreadPoolExecutor:
    ...
    if _cpu_executor is None:
        ...
        _cpu_executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="textkit-cpu"
        )
...
class AsyncTextTransformationEngine:
    ...
    async def transform_async(
        self, text: str, rule_string: str, enable_streaming: bool = False
        ...
        try:
            ...
            request = TextTransformationRequest(text=text, rule_string=rule_string)
    ...
    async def _transform_streaming(
        self, request: TextTransformationRequest
    ) -> AsyncIterator[str]:
    ...
    async def transform_batch_async(self, requests: list[tuple[str, str]]) -> list[str]:
        ...
        try:
            ...
            tasks = [self.transform_async(text, rules) for text, rules in requests]
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error("batch_item_failed", item_index=i, error=str(result))
            ...
            logger.error(
                "batch_transformation_failed", error=str(e), batch_size=len(requests)
            )
            raise
